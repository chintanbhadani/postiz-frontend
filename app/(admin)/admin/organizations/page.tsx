...
export default function AdminOrganizationsPage() {
  ...
  const fetchOrgs = async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/admin/organizations", { params: { page, limit, search: debouncedSearch, status } });
      setOrgs(data.data);
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm">Plan Status</th>
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm text-center">Members</th>
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm text-center">Channels</th>
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm text-center">Posts</th>
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm">Created</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="py-8 text-center text-gray-500">Loading organizations...</td>
              </tr>
            ) : orgs.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-8 text-center text-gray-500">No organizations found</td>
              </tr>
            ) : (
              orgs.map((org) => (
                <tr key={org.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4">
                    ...
                    <div className="text-xs text-gray-400 mt-1">ID: {org.id}</div>
                  </td>
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-md text-xs font-bold ${
                      org.subscriptionStatus === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
                    }`}>
                      {org.subscriptionStatus === "active" ? "PRO" : "FREE"}
                    </span>
                  </td>
