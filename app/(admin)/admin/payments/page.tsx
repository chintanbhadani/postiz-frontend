...
export default function AdminPaymentsPage() {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/admin/payments", { params: { page, limit, search: debouncedSearch, status } });
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm">Payment ID</th>
              <th className="py-3 px-4 font-semibold text-gray-600 text-sm">Amount</th>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-gray-500">Loading payments...</td>
              </tr>
            ) : payments.length === 0 ? (
              <tr>
              ...
              payments.map((payment) => (
                <tr key={payment.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4">
                    <div className="font-medium text-gray-900">{payment.organization?.name || "Unknown"}</div>
                  </td>
                  <td className="py-3 px-4">
                    <div className="text-xs text-gray-500 font-mono">{payment.razorpayPaymentId || "N/A"}</div>
                  </td>
                  ...
                  <td className="py-3 px-4">
                    ...
                    {payment.failureReason && (
                      <div className="text-[10px] text-red-500 mt-1 max-w-[200px] truncate" title={payment.failureReason}>
                        {payment.failureReason}
                      </div>
                    )}
                  </td>
